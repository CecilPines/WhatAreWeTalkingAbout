...
from Reading.video_analyzer.config import Config, get_client, get_model
from Reading.video_analyzer.frame import VideoProcessor
from Reading.video_analyzer.prompt import PromptLoader
from Reading.video_analyzer.analyzer import VideoAnalyzer
from Reading.video_analyzer.audio_processor import AudioProcessor, AudioTranscript
from Reading.video_analyzer.clients.ollama import OllamaClient
from Reading.video_analyzer.clients.generic_openai_api import GenericOpenAIAPIClient
# --- [新增导入] ---
# 导入我们创建的翻译模块
from Reading.video_analyzer.translator import translate_summary_to_chinese
# --- [新增导入结束] ---
...
def video_main(file_path):
    ...
    video_path = file_path #"E:/Program/Python/Program/SinaTopic-Test/Reading/test.mp4" # Path(args.video_path)
    ...
    try:
        ...
        if args.start_stage <= 1:
            ...
            if audio_path is None:
                ...
                print("No audio found in video - skipping transcription")
                transcript = None
            else:
            ...
            print(f"Extracting frames from video using model {model}...")
        ...
        if args.start_stage <= 3:
            ...
            print("Reconstructing video description...")
        ...
        if video_description and video_description.get("response"):
            logger.info("Translating video description to Chinese...")
            print("Translating video description to Chinese...")
            
            # 从配置中安全地获取 Ollama URL
            ollama_url = config.get("clients", {}).get("ollama", {}).get("url", "http://localhost:11434")
            # 使用 get_model 函数获取当前配置的模型 (例如 llama3.2-vision)
            translation_model = get_model(config) 
            
            # 准备 translator.py 所需的字典
        ...
        results = {
            ...
            "video_description": video_description # <-- 此处现在包含中文翻译
        }
        ...
        # if not config.get("keep_frames"): # 11.11新增，将任务结束后删除帧注释掉了，保留帧，待后续一并删除
...
if __name__ == "__main__":
    file_path = "E:/Program/Python/Program/SinaTopic-Test/Reading/test.mp4"
