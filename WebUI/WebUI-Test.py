...
from test_paddle import process_ocr, process_images_in_parallel
from Reading.video_analyzer.cli import video_main
from Reading.video_analyzer.video_analyzer import analyze_video_with_qwen_local

from Picturing.weibo_user_picture import get_weibo_users_pic
from Picturing.word_cloud import get_generated_wordcloud

from Analyzing.step1_cut_words.test_cut_words import get_comments, get_data, extract_key, clean_weibo, get_emotional_tendency
...
def Spidering():
    ...
    result_data["page_text_1"] = []
    result_data["page_text_2"] = []
    # 安全地添加微博内容
    if not df_weibo.empty:
        weibo = df_weibo["内容"][0]
        result_data["page_text_1"].append(weibo)
        print(weibo)
    # 安全地添加评论内容
    if not df_comments.empty:
        for index, row in df_comments.iterrows():
            result_data["page_text_2"].append(row.tolist())
...
def Reading():
    ...
    try:
        if image_flag:
            ...
            try:
                max_threads = 3
                ocr_result = process_images_in_parallel(image_path_list, max_threads) # process_ocr(image_path)
                clean_result = clean_weibo(ocr_result, os.path.join(get_output_dir(), "image_reading.txt"))
                # result_data["reading_texts"].append(ocr_result)
                result_data["reading_texts"].append(clean_result)
        ...
        if video_flag:
            ...
            for video in result_data["page_videos"]:
                ...
                try:
                    ...
                    video_result = analyze_video_with_qwen_local(video_path)
...
def Analyzing():
    ...
    print("1/5 - 2/4 分析微博内容引导倾向...")
    result = get_emotional_tendency(os.path.join(get_output_dir(), "weibo.txt"))
    result_data["analysing_texts"].append(result)

    print("1/5 - 3/4 词转换开始...")
    get_data(os.path.join(get_output_dir(), "weibo.txt"), os.path.join(get_output_dir(), "weibo_cut.dat"))
    print("1/5 - 4/4 关键词提取开始...")
    ...
    result_data["analysing_imgs"].append("weibo_keywords.png")
    # result_data["analysing_imgs"].append("weibo_sentiment_distribution.png")

    result_data["analysing_imgs"].append("comments_keywords.png")
    result_data["analysing_imgs"].append("comment_repost_time_plots.png")
    result_data["analysing_imgs"].append("comments_sentiment_distribution.png")
    result_data["analysing_imgs"].append("emotion_distribution.png")
...
def Picturing():
    ...
    result_data["picturing_imgs"] = ["user_word_cloud.png"]
...
def Run(keyword):
    ...
    try:
        ...
        print("\n[3/4] 执行 Analyzing 阶段...")
        Analyzing()
        print(f"Analyzing 完成，progress step: {progress['step']}, status: {progress['status']}")

        print("\n[4/4] 执行 Picturing 阶段...")
        Picturing()
        # time.sleep(2)
        print(f"Picturing 完成，progress step: {progress['step']}, status: {progress['status']}")
...
@app.route('/reset')
def reset():
    """清空中间结果（保留 progress.js）并返回首页"""
    global progress, result_data, picture_users_list, video_flag, image_flag

    # 获取 static 目录路径
    static_dir = os.path.join(app.root_path, 'static')

    # 删除 static 文件夹下除 progress.js 以外的所有文件
    ...
    # 获取 Spidering 中间结果路径
    spidering_dir = os.path.join(get_project_root(), 'Spidering', 'backend', 'weibo_analysis')
    # 删除 weibo_analysis 文件夹下所有文件
    if os.path.exists(spidering_dir):
        for filename in os.listdir(spidering_dir):
            file_path = os.path.join(spidering_dir, filename)
            try:
                if os.path.isfile(file_path) or os.path.islink(file_path):
                    os.remove(file_path)
                elif os.path.isdir(file_path):
    ...
    # 获取 Reading 中间结果路径
    reading_dir = os.path.join(get_project_root(), 'Reading', 'video_analyzer', 'output')
    # 删除 output 文件夹下所有文件
    if os.path.exists(reading_dir):
        for filename in os.listdir(reading_dir):
            ...
            try:
                ...
                print(f"❌ 删除 {file_path} 时出错: {e}")
    reading_dir_2 = os.path.join(get_project_root(), 'WebUI', 'output')
    # 删除 output 文件夹下所有文件
    if os.path.exists(reading_dir_2):
        for filename in os.listdir(reading_dir_2):
            file_path = os.path.join(reading_dir_2, filename)
            try:
                if os.path.isfile(file_path) or os.path.islink(file_path):
                    os.remove(file_path)
                elif os.path.isdir(file_path):
    ...
    print("✅ 已清理 static 文件夹（保留 progress.js）并重置全局状态。")
    return redirect(url_for('index'))
