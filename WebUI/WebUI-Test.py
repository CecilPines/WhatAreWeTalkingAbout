...
def Spidering():
    ...
    try:
        df_comments = pandas.read_csv(comments_file, encoding='utf-8') if os.path.exists(
            comments_file) else pandas.DataFrame()
    except Exception as e:
        df_comments = pandas.DataFrame()
        print(f"警告：读取评论文件失败: {e}")

    try:
        df_repost = pandas.read_csv(reposts_file, encoding='utf-8') if os.path.exists(
            reposts_file) else pandas.DataFrame()
    except Exception as e:
...
def Analyzing():
    ...
    print("1/3 - 1/3 评论合并开始...")
    get_comments(csv_path)
    print("1/3 - 2/3 词转换开始...")
    get_data()
    print("1/3 - 3/3 关键词提取开始...")
    extract_key()
    print("1/3 正在绘制词云图...")
    get_word_cloud()

    print("2/3 正在分析评论情感倾向...")
    target_path = os.path.join(get_output_dir(), "comments.txt")
    get_predict(target_path)
    result_data["analysing_imgs"] = ["keywords.png", "comment_repost_time_plots.png", "sentiment_distribution.png"]
