...
def process_images_in_parallel(image_paths, max_threads=4):
    """并行处理多个图像，支持最大线程数限制"""
    result_list = []
    with ThreadPoolExecutor(max_workers=max_threads) as executor:
        futures = {executor.submit(process_ocr, img_path): img_path for img_path in image_paths}
...
# 示例调用
image_paths = [os.path.join(get_output_dir(), wid+"_1.jpg"),os.path.join(get_output_dir(), wid+"_2.jpg"),os.path.join(get_output_dir(), wid+"_3.jpg"),os.path.join(get_output_dir(), wid+"_4.jpg"),os.path.join(get_output_dir(), wid+"_5.jpg")]  # 你可以添加更多图片路径
max_threads = 3
process_images_in_parallel(image_paths,max_threads)"""
