...
def get_predict(target_path):
    ...
    with open(target_path, "r", encoding="utf-8") as f:
        for line in f:
            ...
            max_seq_len = 512
            try:
                results = predict(input_text, ext_model, cls_model, tokenizer, ext_id2label, cls_id2label, max_seq_len=max_seq_len)
                if results:
                ...
                continue
    # 保存opinions
    opinions_file = os.path.join(get_output_dir(), "opinions.txt")
    ...
    # 保存饼图到文件
    pie_path = os.path.join(get_output_dir(), "sentiment_distribution.png")
...
if __name__== "__main__" :
    get_predict(os.path.join(get_output_dir(), "comments.txt"))
